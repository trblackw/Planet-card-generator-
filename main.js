...
//Creates card HTML w/ selected planet info
function createPlanetHTML(planet) {
  return `<div class='card col-sm-11 col-md-9 col-lg-7 col-xl-6 m-auto'>
    ...
    <h3>Planet Profile</h3>
        ...
        <p class='lead mt-3 mb-0'id='nasaLink'><em>For more information on ${planet.name}, here's what <a href='https://solarsystem.nasa.gov/planets/overview/' target='_blank'>NASA</a> has to say</p>
      </div>
      <div class='container text-center'>
      <a class='btn btn-info w-40 mt-3' href='index.html' role='button'>Go back to planet selection</a>
      </div>`
  }
...
const selectStep1 = document.getElementsByTagName('select');
...
//paragraph handlers 
earth.addEventListener('click', () => body.innerHTML = createPlanetHTML(Earth));
mars.addEventListener('click', () => body.innerHTML = createPlanetHTML(Mars));
jupiter.addEventListener('click', () => body.innerHTML = createPlanetHTML(Jupiter));
uranus.addEventListener('click', () => body.innerHTML = createPlanetHTML(Uranus));
neptune.addEventListener('click', () => body.innerHTML = createPlanetHTML(Neptune));
